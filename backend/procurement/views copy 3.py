        ...
        for index, row in df.iterrows():
            try:
                ...
                supplier = Supplier(
                    ...
                    contact_person=str(row.get('联系人', '')) if pd.notna(row.get('联系人')) else '',
                    contact_phone=str(row.get('联系电话', '')) if pd.notna(row.get('联系电话')) else '',
                    contact_email=str(row.get('邮箱', '')) if pd.notna(row.get('邮箱')) else '',
...
class PurchaseOrderViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if expected_date_start and expected_date_end:
            queryset = queryset.filter(expected_date__range=[expected_date_start, expected_date_end])
        ...
        # 实际到货日期范围
        actual_receive_date_start = self.request.query_params.get('actual_receive_date_start')
        actual_receive_date_end = self.request.query_params.get('actual_receive_date_end')
        if actual_receive_date_start and actual_receive_date_end:
            queryset = queryset.filter(actual_receive_date__range=[actual_receive_date_start, actual_receive_date_end])
        elif actual_receive_date_start:
            queryset = queryset.filter(actual_receive_date__gte=actual_receive_date_start)
    ...
    def batch_submit(self, request):
        ...
        updated = PurchaseOrder.objects.filter(id__in=ids, status='draft').update(status='submitted')
    ...
    def batch_approve(self, request):
        ...
        count = PurchaseOrder.objects.filter(id__in=ids, status='submitted').update(status='approved')
        return Response({'success': count, 'message': f'成功审核 {count} 个订单'})

    @action(detail=False, methods=['post'])
    def batch_review(self, request):
        ids = request.data.get('ids', [])
        count = PurchaseOrder.objects.filter(id__in=ids, status='approved').update(status='reviewed')
        return Response({'success': count})

    @action(detail=False, methods=['post'])
    def batch_final_approve(self, request):
        ids = request.data.get('ids', [])
        count = PurchaseOrder.objects.filter(id__in=ids, status='reviewed').update(status='final_approved')
        return Response({'success': count})

    @action(detail=False, methods=['post'])
    def batch_receive(self, request):
        ids = request.data.get('ids', [])
        count = PurchaseOrder.objects.filter(id__in=ids, status='final_approved').update(status='received')
        return Response({'success': count})

    @action(detail=False, methods=['post'])
    def batch_inspect(self, request):
        ids = request.data.get('ids', [])
        count = PurchaseOrder.objects.filter(id__in=ids, status='received').update(status='inspected')
        return Response({'success': count})

    @action(detail=False, methods=['post'])
    def batch_store(self, request):
        ids = request.data.get('ids', [])
        count = PurchaseOrder.objects.filter(id__in=ids, status='inspected').update(status='stored')
        return Response({'success': count})

    @action(detail=False, methods=['post'])
    def batch_close(self, request):
        ids = request.data.get('ids', [])
        count = PurchaseOrder.objects.filter(id__in=ids, status='stored').update(status='closed')
        return Response({'success': count})
    ...
    def export(self, request):
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            if detail_data:
                ...
                pd.DataFrame({'提示': ['所选订单无商品明细']}).to_excel(writer, sheet_name='商品明细', index=False)
    ...
    def import_excel(self, request):
        ...
        for idx, row in df_orders.iterrows():
            try:
                ...
                if not supplier:
                    errors.append(f'第{idx+2}行：找不到供应商 (编码:{supplier_code} 名称:{supplier_name} 供应商列:{supplier_col})，请确保供应商已录入系统')
                    continue

                order_date = row.get('下单日期') if pd.notna(row.get('下单日期')) else None
                expected_date = row.get('预计到货日期') if pd.notna(row.get('预计到货日期')) else None
                actual_receive_date = row.get('实际到货日期') if pd.notna(row.get('实际到货日期')) else None
                status_value = str(row.get('状态', '草稿')).strip()
                order_status = status_map.get(status_value, 'draft')
                total_amount = float(row.get('总金额', 0)) if pd.notna(row.get('总金额')) else 0
                ...
                if not df_items.empty:
                    ...
                    for _, item_row in order_items.iterrows():
                        ...
                        if not material:
                            errors.append(f'订单 {po_no} 物料编码 {material_code} 不存在，跳过明细')
                            continue
                        quantity = float(item_row.get('数量', 0)) if pd.notna(item_row.get('数量')) else 0
                        unit_price = float(item_row.get('单价', 0)) if pd.notna(item_row.get('单价')) else 0
                        amount = float(item_row.get('金额', 0)) if pd.notna(item_row.get('金额')) else quantity * unit_price
                        PurchaseOrderItem.objects.create(
                            ...
                            amount=amount,
                            expected_date=item_row.get('期望到货日期') if pd.notna(item_row.get('期望到货日期')) else None
                        )
