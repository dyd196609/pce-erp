...
class PurchaseOrderViewSet(viewsets.ModelViewSet):
    ...
    def batch_approve(self, request):
        ...
        count = PurchaseOrder.objects.filter(
            id__in=ids, status='submitted').update(status='approved')
    ...
    def batch_review(self, request):
        ids = request.data.get('ids', [])
        count = PurchaseOrder.objects.filter(
            id__in=ids, status='approved').update(status='reviewed')
    ...
    def batch_final_approve(self, request):
        ids = request.data.get('ids', [])
        count = PurchaseOrder.objects.filter(
            id__in=ids, status='reviewed').update(status='final_approved')
        return Response({'success': count})
    ...
    def batch_receive(self, request):
        ids = request.data.get('ids', [])
        count = PurchaseOrder.objects.filter(
            id__in=ids, status='final_approved').update(status='received')
        return Response({'success': count})
    ...
    def batch_inspect(self, request):
        ids = request.data.get('ids', [])
        count = PurchaseOrder.objects.filter(
            id__in=ids, status='received').update(status='inspected')
        return Response({'success': count})
    ...
    def batch_store(self, request):
        ids = request.data.get('ids', [])
        count = PurchaseOrder.objects.filter(
            id__in=ids, status='inspected').update(status='stored')
        return Response({'success': count})
    ...
    def batch_close(self, request):
        ids = request.data.get('ids', [])
        count = PurchaseOrder.objects.filter(
            id__in=ids, status='stored').update(status='closed')
        return Response({'success': count})
        ...
        for order in queryset:
            for item in order.items.all():
                detail_data.append({
                    ...
                    '计划金额': item.amount,
                    '实际交货数量': item.actual_quantity if item.actual_quantity is not None else '',
                    '实际交货单价': item.actual_unit_price if item.actual_unit_price is not None else '',
                    '实际交货金额': item.actual_amount if item.actual_amount is not None else '',
                    '实际到货日期': item.actual_arrival_date.strftime('%Y-%m-%d') if item.actual_arrival_date else '',
                    '期望到货日期': item.expected_date.strftime('%Y-%m-%d') if item.expected_date else ''
                })
            ...
            try:
                ...
                if not df_items.empty:
                    ...
                    for _, item_row in order_items.iterrows():
                        ...
                        quantity = float(item_row.get('计划数量', 0)) if pd.notna(
                            item_row.get('计划数量')) else 0
                        unit_price = float(item_row.get('计划单价', 0)) if pd.notna(
                            item_row.get('计划单价')) else 0
                        amount = float(item_row.get('计划金额', 0)) if pd.notna(
                            item_row.get('计划金额')) else quantity * unit_price

                        actual_quantity = float(item_row.get('实际交货数量', 0)) if pd.notna(
                            item_row.get('实际交货数量')) else None
                        actual_unit_price = float(item_row.get('实际交货单价', 0)) if pd.notna(
                            item_row.get('实际交货单价')) else None
                        actual_amount = float(item_row.get('实际交货金额', 0)) if pd.notna(item_row.get('实际交货金额')) else (
                            actual_quantity * actual_unit_price if actual_quantity and actual_unit_price else None)
                        actual_arrival_date = item_row.get('实际到货日期') if pd.notna(
                            item_row.get('实际到货日期')) else None
