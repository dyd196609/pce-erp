<template>
  <div style="padding: 20px">
    ...
    <!-- 表格（横向滚动） -->
    <div class="table-scroll-container" ref="tableScrollContainer" @mousemove="onTableMouseMove"
      @mouseleave="onTableMouseLeave">
      <el-table ref="multipleTable" :data="orderList" border stripe height="calc(100vh - 280px)"
        style="min-width: 1400px" @selection-change="handleSelectionChange" @sort-change="handleSortChange">
        ...
        <el-table-column prop="supplier_name" label="供应商" min-width="150" sortable="custom" show-overflow-tooltip>
          <template #header>
            <div>
              <div>供应商</div>
              <div style="display: flex; gap: 4px; margin-top: 4px">
                <el-input v-model="filters.supplier_name" placeholder="输入筛选" size="small" clearable
        ...
        <el-table-column prop="order_date" label="下单日期" width="260" sortable="custom">
          <template #header>
            <div>
              ...
              <div style="display: flex; gap: 4px; margin-top: 4px">
                <el-date-picker v-model="dateRange.order_date" type="daterange" range-separator="至"
        ...
        <el-table-column prop="expected_date" label="预计到货日期" width="260" sortable="custom">
          <template #header>
            <div>
              ...
              <div style="display: flex; gap: 4px; margin-top: 4px">
                <el-date-picker v-model="dateRange.expected_date" type="daterange" range-separator="至"
                  start-placeholder="开始日期" end-placeholder="结束日期" size="small" value-format="YYYY-MM-DD"
                  :unlink-panels="true" :default-value="new Date()" @change="handleDateRangeFilter" style="flex: 1" />
        ...
        <el-table-column prop="actual_receive_date" label="实际到货日期" width="260" sortable="custom">
          <template #header>
            <div>
              ...
              <div style="display: flex; gap: 4px; margin-top: 4px">
                <el-date-picker v-model="dateRange.actual_receive_date" type="daterange" range-separator="至"
                  start-placeholder="开始日期" end-placeholder="结束日期" size="small" value-format="YYYY-MM-DD"
                  :unlink-panels="true" :default-value="new Date()" @change="handleDateRangeFilter" style="flex: 1" />
        ...
        <el-table-column prop="total_amount" label="总金额" width="150" sortable="custom">
          <template #header>
            <div>
              <div>总金额</div>
              <div style="display: flex; gap: 4px; margin-top: 4px">
                <el-input v-model="amountMin" placeholder="最低" size="small" style="width: 45%"
                  @input="handleAmountFilter" />
                <span>-</span>
                <el-input v-model="amountMax" placeholder="最高" size="small" style="width: 45%"
                  @input="handleAmountFilter" />
        ...
        <el-table-column prop="status" label="状态" width="120" sortable="custom">
          <template #header>
            <div>
              <div>状态</div>
              <div style="margin-top: 4px; display: flex; gap: 4px; align-items: center;">
                <el-select v-model="filters.status" placeholder="选择状态" size="small" clearable
                  @change="handleSelectFilter" style="flex: 1;">
                  <el-option label="草稿" value="draft" />
                  <el-option label="已提交" value="submitted" />
                  <el-option label="已审核" value="approved" />
                  <el-option label="已复核" value="reviewed" />
                  <el-option label="已审批" value="final_approved" />
        ...
        <el-table-column label="操作" fixed="right" width="360">
          <template #default="{ row }">
            <div style="display: flex; flex-wrap: wrap; gap: 4px;">
              ...
              <el-button v-if="row.status !== 'cancelled' && row.status !== 'closed'" link type="warning"
              ...
              <!-- 流程按钮-->
              <!-- 提交（草稿） -->
              <el-button v-if="row.status === 'draft'" link type="success" @click="submitOrder(row)">提交</el-button>
              <!-- 审核（已提交） -->
              <el-button v-if="row.status === 'submitted'" link type="success" @click="approveOrder(row)">审核</el-button>
              <!-- 复核（已审核） -->
              <el-button v-if="row.status === 'approved'" link type="success" @click="reviewOrder(row)">复核</el-button>
              <!-- 审批（已复核） -->
              <el-button v-if="row.status === 'reviewed'" link type="success"
                @click="finalApproveOrder(row)">审批</el-button>
              <!-- 收货（已审批） -->
              <el-button v-if="row.status === 'final_approved'" link type="success"
                @click="receiveOrder(row)">收货</el-button>
              <!-- 检验（已收货） -->
              <el-button v-if="row.status === 'received'" link type="success" @click="inspectOrder(row)">检验</el-button>
              <!-- 入库（已检验） -->
              <el-button v-if="row.status === 'inspected'" link type="success" @click="storeOrder(row)">入库</el-button>
              <!-- 结案（已入库） -->
              <el-button v-if="row.status === 'stored'" link type="success" @click="closeOrder(row)">结案</el-button>
              <el-button v-if="row.status === 'cancelled'" link type="success" @click="restartOrder(row)">重启</el-button>
            </div>
          </template>
        </el-table-column>
      </el-table>
          ...
          <el-table :data="form.items" border>
            <el-table-column label="物料" width="250">
              <template #default="{ row, $index }">
                <el-select v-model="row.material_id" filterable remote :remote-method="() => { }" placeholder="选择物料"
                  style="width: 100%" @change="() => onMaterialChange($index)">
            ...
            <el-table-column label="单价" width="120">
              <template #default="{ row, $index }">
                <el-input-number v-model="row.unit_price" :min="0" :precision="2" controls-position="right"
                  @change="() => calcItemAmount($index)" />
              </template>
            </el-table-column>
    ...
    <el-dialog v-model="viewDialogVisible" title="订单详情" width="800px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="状态">{{ statusMap[viewData.status] || viewData.status_display || viewData.status
...
const cancelOrder = async (row) => {
  if (row.status === 'cancelled' || row.status === 'closed') {
    ElMessage.warning('该订单已取消或已结案，无法再次取消')
    return
  }
...
const reviewOrder = async (row) => {
  ...
  try {
    ...
    ElMessage.error(e.response?.data?.error || '复核失败')
  }
}
...
const receiveOrder = async (row) => {
  ...
  try {
    ...
    ElMessage.error(e.response?.data?.error || '收货失败')
  }
}
...
const inspectOrder = async (row) => {
  ...
  try {
    ...
    ElMessage.error(e.response?.data?.error || '检验失败')
  }
}
...
const closeOrder = async (row) => {
  ...
  try {
    ...
    ElMessage.error(e.response?.data?.error || '结案失败')
  }
}
...
const loadUserPermissions = async () => {
  try {
    const res = await request.get('/auth/user/permissions/')
