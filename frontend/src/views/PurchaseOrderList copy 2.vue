<template>
  <div style="padding: 20px">
    ...
    <div class="table-scroll-container" ref="tableScrollContainer" @mousemove="onTableMouseMove"
      ...
      <el-table ref="multipleTable" :data="orderList" border stripe style="min-width: 1400px"
        ...
        <!-- 下单日期范围 + 清除按钮 + 独立面板 -->
        <el-table-column prop="order_date" label="下单日期" width="260">
          <template #header>
            <div>
              <div>下单日期</div>
              <div style="display: flex; gap: 4px; margin-top: 4px">
        ...
        <!-- 预计到货日期范围 + 清除按钮 + 独立面板 -->
        <el-table-column prop="expected_date" label="预计到货日期" width="260">
          <template #header>
            <div>
              <div>预计到货日期</div>
              <div style="display: flex; gap: 4px; margin-top: 4px">
        ...
        <!-- 实际到货日期范围 + 清除按钮 + 独立面板 -->
        <el-table-column prop="actual_receive_date" label="实际到货日期" width="260">
          <template #header>
            <div>
              <div>实际到货日期</div>
              <div style="display: flex; gap: 4px; margin-top: 4px">
        ...
        <el-table-column prop="status_display" label="状态" width="120" sortable="custom">
          <template #header>
            <div>
              <div>状态</div>
    ...
    <el-dialog v-model="viewDialogVisible" title="订单详情" width="800px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="状态">{{ viewData.status_display || viewData.status }}</el-descriptions-item>
...
const submitForm = async () => {
  ...
  try {
    const submitData = {
      ...form.value,
      supplier_id: Number(form.value.supplier_id),   // 强制转为数字
      company_id: 1,
      buyer: employeeList.value.find(e => e.id === form.value.buyer_id)?.full_name || form.value.buyer,
