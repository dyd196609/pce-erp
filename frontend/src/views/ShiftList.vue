  ...
  <div style="padding: 20px">
    ...
    <div style="overflow-x: auto; width: 100%">
      <el-table :data="shiftList" border stripe>
        <el-table-column v-for="col in config.columns" :key="col.field" :prop="col.field" :label="col.title"
          :width="col.width">
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="500px">
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        <el-form-item v-for="col in config.columns" :key="col.field" :label="col.title" :prop="col.field"
          :required="col.required">
          <el-input v-if="col.type === 'text'" v-model="form[col.field]" :placeholder="`请输入${col.title}`" />
          <el-select v-else-if="col.type === 'select'" v-model="form[col.field]" :placeholder="`请选择${col.title}`">
            <el-option v-for="opt in col.options" :key="opt.value" :label="opt.label" :value="opt.value" />
          </el-select>
          <el-time-picker v-else-if="col.type === 'time'" v-model="form[col.field]" format="HH:mm:ss"
            value-format="HH:mm:ss" :placeholder="`请选择${col.title}`" />
          <el-switch v-else-if="col.type === 'boolean'" v-model="form[col.field]" active-text="启用" inactive-text="禁用" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const loadShifts = async () => {
  try {
    const res = await request.get('/api/pfm/shifts/');
...
const submitForm = async () => {
  ...
  try {
    if (form.value.id) {
      ...
      await request.post('/api/pfm/shifts/', form.value);
